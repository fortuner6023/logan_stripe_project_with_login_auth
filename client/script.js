...
fetch("/setup")
  ...
  .then(function (json) {
    ...
    var membershipMonthlyPriceId = json.membershipMonthly;
    ...
    // Setup event handler to create a Checkout Session when button is clicked
    document
      .getElementById("membership-yearly-btn")
      .addEventListener("click", function (evt) {
        createCheckoutSession(membershipYearlyPriceId).then(function (data) {
          // Call Stripe.js method to redirect to the new Checkout page
          stripe
            .redirectToCheckout({
              sessionId: data.sessionId,
